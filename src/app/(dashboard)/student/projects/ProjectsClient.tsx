...
export default function ProjectsClient({ userId, initialProjects, initialSubs }: Props) {
  ...
  const ToastEl = () => toast ? <div style={{ position:'fixed', top:70, left:'50%', transform:'translateX(-50%)', zIndex:200, background:T.emerald, color:'#fff', padding:'10px 20px', borderRadius:30, fontSize:13, fontWeight:600, whiteSpace:'nowrap', pointerEvents:'none' }}>{toast}</div> : null
    ...
    <div style={{ fontFamily:'Inter,sans-serif', background:T.bg, minHeight:'100vh', padding:'16px 16px 32px' }}>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))}
          ...
          rows={5} style={{ width:'100%',padding:'11px 14px',background:T.navy2,border:`1px solid ${form.notes?T.indigo:T.border}`,borderRadius:10,color:T.white,fontSize:13,resize:'none',boxSizing:'border-box',fontFamily:'Inter,sans-serif',lineHeight:1.6 }} />
      </div>
      ...
      <div style={{ fontFamily:'Inter,sans-serif', background:T.bg, minHeight:'100vh' }}>
        ...
        <div style={{ padding:'0 16px', borderBottom:`1px solid ${T.border}`, paddingBottom:16, marginBottom:16 }}>
          <div style={{ display:'flex',gap:8,marginBottom:10,flexWrap:'wrap' }}>
            <span style={{ fontSize:11,padding:'3px 10px',borderRadius:20,background:`${diff.color}20`,color:diff.color }}>{diff.label}</span>
            <span style={{ fontSize:11,padding:'3px 10px',borderRadius:20,background:st.bg,color:st.color,border:`1px solid ${st.color}30` }}>{st.emoji} {st.label}</span>
            <span style={{ fontSize:11,padding:'3px 10px',borderRadius:20,background:T.navy2,color:T.slate,border:`1px solid ${T.border}` }}>~{active.estimated_hours}</span>
          </div>
        ...
        <div style={{ padding:'0 16px 32px' }}>
          ...
          {sub?.github_url && (
            <div style={{ background:T.navy2,border:`1px solid ${T.border}`,borderRadius:14,padding:16,marginBottom:12 }}>
              ...
              <a href={sub.github_url} target="_blank" rel="noreferrer" style={{ display:'flex',alignItems:'center',gap:8,fontSize:14,color:T.indigo,textDecoration:'none',marginBottom:sub.live_url?8:0 }}>🐙 <span>{sub.github_url}</span></a>
              {sub.live_url && <a href={sub.live_url} target="_blank" rel="noreferrer" style={{ display:'flex',alignItems:'center',gap:8,fontSize:14,color:T.emerald,textDecoration:'none',marginBottom:sub.notes?8:0 }}>🌐 <span>{sub.live_url}</span></a>}
              {sub.notes && <div style={{ fontSize:13,color:T.slate,lineHeight:1.6,fontStyle:'italic' }}>&ldquo;{sub.notes}&rdquo;</div>}
            </div>
          )}
              ...
              {(['not_started','in_progress'] as const).map(s=>{
                const st2=STATUS[s]; return (
                  <button key={s} onClick={()=>updateStatus(s)} disabled={saving||status===s}
                    style={{ flex:1,padding:'10px 6px',borderRadius:10,border:`1px solid ${status===s?st2.color+'60':T.border}`,background:status===s?st2.bg:T.navy2,color:status===s?st2.color:T.slate,cursor:status===s?'default':'pointer',fontSize:12,fontWeight:600 }}>
                    {st2.emoji} {st2.label}
                  </button>
                )
              })}
          ...
          {status!=='approved' && (
            <button onClick={()=>openSubmit(active)}
              style={{ width:'100%',padding:15,background:status==='submitted'?T.navy2:T.indigo,border:status==='submitted'?`1px solid ${T.border}`:'none',borderRadius:12,color:status==='submitted'?T.slate:'#fff',fontSize:15,fontWeight:700,cursor:'pointer' }}>
  ...
  return (
    <div style={{ padding:'16px 16px 8px', fontFamily:'Inter,sans-serif' }}>
      ...
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ fontSize:13,color:T.slate,marginTop:4 }}>{approvedCount} approved · {initialProjects.length} total · Tap to open</div>
      </div>
      <div style={{ background:T.navy2,border:`1px solid ${T.border}`,borderRadius:12,padding:'12px 16px',marginBottom:16 }}>
        <div style={{ display:'flex',justifyContent:'space-between',marginBottom:8 }}>
          ...
          <span style={{ fontSize:13,fontWeight:700,color:T.emerald }}>{approvedCount} / {initialProjects.length}</span>
        </div>
        <div style={{ height:6,background:T.navy3,borderRadius:3 }}>
          <div style={{ height:'100%',borderRadius:3,background:`linear-gradient(90deg,${T.emerald},${T.indigo})`,width:`${initialProjects.length?Math.round((approvedCount/initialProjects.length)*100):0}%` }} />
        </div>
      </div>
      {initialProjects.map(p => {
        ...
        return (
          <div key={p.id} onClick={()=>openDetail(p)}
            style={{ background:st.bg,border:`1px solid ${status==='approved'?T.emerald+'50':status==='submitted'?T.indigo+'50':status==='in_progress'?T.amber+'50':T.border}`,borderRadius:14,padding:'14px 16px',marginBottom:10,cursor:'pointer' }}>
