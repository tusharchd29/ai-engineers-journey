...
export default function TasksClient({ userId, phase, initialTasks, initialCompletions }: Props) {
  ...
  const markComplete = async () => {
    ...
    if (error) { console.error(error); showToast('Error saving — try again'); setSaving(false); return }
    ...
    <div style={{ fontFamily:'Inter,sans-serif', background:T.bg, minHeight:'100vh', padding:'16px 16px 32px' }}>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <textarea value={proofText} onChange={e => setProofText(e.target.value)}
          placeholder={`e.g. Completed "${activeTask.title}" — took notes, pushed to GitHub`}
          rows={4}
      ...
      <div style={{ fontFamily:'Inter,sans-serif', background:T.bg, minHeight:'100vh' }}>
        ...
        <div style={{ padding:'0 16px 32px' }}>
          ...
          {activeTask.description ? (
            ...
            <div style={{ background:T.navy2, border:`1px solid ${T.border}`, borderRadius:14, padding:18, marginBottom:12 }}>
              <div style={{ fontSize:15, color:T.slate, lineHeight:1.8 }}>Complete this task as part of the current phase curriculum. Mark it done when finished.</div>
            </div>
          )}
          ...
          {activeTask.resource_url && (
            <a href={activeTask.resource_url} target="_blank" rel="noreferrer"
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:14, fontWeight:600, color:T.indigoL }}>Open Resource</div>
    ...
    <div style={{ padding:'16px 16px 8px', fontFamily:'Inter,sans-serif' }}>
      ...
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ fontSize:13, color:T.slate, marginTop:4 }}>
          {phase?.label ?? 'Phase 1'} · {doneCount} of {tasks.length} complete
        </div>
      </div>
      ...
      <div style={{ background:T.navy2, border:`1px solid ${T.border}`, borderRadius:12, padding:'14px 16px', marginBottom:20 }}>
        ...
        <div style={{ marginTop:8, fontSize:11, color:T.slate }}>Tap any task → see details → mark complete → upload proof</div>
      </div>
      ...
      {tasks.length === 0 && (
        <div style={{ textAlign:'center', padding:'60px 0', color:T.slate }}>
          ...
          <div style={{ fontSize:13 }}>Check back soon — tasks are being added.</div>
        </div>
      )}
